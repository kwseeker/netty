...
void do_poll(int listenfd)
{
    ...
    struct pollfd clientfds[OPEN_MAX];      //可接受的client最大值（实际最大值取OPEN_MAX和系统支持最大文件句柄中的最小值）, select最多1024
    int maxfd;
    int i;
    int nready;
    ...
    while(1)
    {
        nready = poll(clientfds, maxfd+1, INFTIM);
        if(nready == -1)
        {
            perror("poll error:");
            exit(1);
